...
def kalman_step(A, C, Q, R, y, x, V):
    # INPUTS:
    # A - the system matrix (Fi)
    # C - the observation matrix (H)
    # Q - the system covariance
    # R - the observation covariance
    # y(:) - the observation (measures) at time t
    # x(:) - E[X | y(:, 1:t-1)] prior mean (expected value of the prior)
    # V(:,:) - Cov[X | y(:, 1:t-1)] prior covariance

    # COMMENTS:
    # The posterior p(x_k|y_(1:k)) is computes as the product of:
    # - observation model: p(y_k|[x_k, y_(1:k-1)]) == p(y_k|x_k)
    # - prior pdf: p(x_k|y_(1:k-1))
    # the prior is computed as:
    # sum(p(x_k|[x_(k-1),y_(1:k:1)])*p(x_(k-1)|y_(1:k:1)))*d{x_(k-1)}
    # where:
    # - p(x_k|[x_(k-1),y_(1:k:1)]) == p(x_k|x_(k-1)) (the dynamic model)
    # - p(x_(k-1)|y_(1:k:1)) (the posterior at the previous time step)

    # OUTPUTS (where X is the hidden state being estimated)
    # xnew(:) =   E[ X | y(:, 1:t) ] (expected value of the posterior)
    # Vnew(:,:) = Var[ X(t) | y(:, 1:t) ] (variance of the posterior)
    # VVnew(:,:) = Cov[ X(t), X(t-1) | y(:, 1:t) ] (covariance)
...
def extract_histogram(patch, n_bins, weights=None):
    ...
    if weights is not None:
        # print('bin_idxs:', len(bin_idxs.flatten()))
        # print('weights:', len(weights.flatten()))
        # if len(bin_idxs.flatten()) != len(weights.flatten()):
        #       print('bug')
